-- Corrige buckets diarios (timestamps inválidos con generate_series + concat).
...
as $$
  ...
  hourly_buckets as (
    select date_trunc('hour', gs) as bucket_at, true as is_hourly
    from bounds b
  ...
  daily_buckets as (
    select d::date as bucket_day, false as is_hourly
    from bounds b
    cross join generate_series(
  ...
  chart_buckets as (
    select bucket_at, is_hourly, null::date as bucket_day from hourly_buckets
    union all
    select null::timestamptz as bucket_at, is_hourly, bucket_day from daily_buckets
  )
  select
      ...
      (
        select jsonb_agg(
          jsonb_build_object(
            'date',
            case
              when cb.is_hourly then to_char(cb.bucket_at at time zone 'utc', 'YYYY-MM-DD"T"HH24:00:00"Z"')
              else cb.bucket_day::text
            end,
            'visits', coalesce(v.cnt, 0)
          )
          order by coalesce(cb.bucket_at, cb.bucket_day::timestamptz)
        )
        ...
        left join (
          select
            case
              ...
              else (bv.visited_at at time zone 'utc')::date
            end as bucket_key,
            count(*)::bigint as cnt
          from public.business_visits bv, bounds
          ...
          (cb.is_hourly and v.bucket_key = cb.bucket_at)
          or (not cb.is_hourly and v.bucket_key = cb.bucket_day)
        )
      ),
      '[]'::jsonb
    ) as daily_visits,
    coalesce(
      (
        ...
        left join (
          select
            case
              when (select days from bounds) = 1 then date_trunc('hour', rev.created_at)
              else (rev.created_at at time zone 'utc')::date
            end as bucket_key,
            count(*)::bigint as cnt
          from public.reviews rev, bounds
          where rev.business_id = mb.id
            and rev.created_at >= timezone('utc', now()) - make_interval(days => bounds.days)
          group by 1
        ) rev on (
          (cb.is_hourly and rev.bucket_key = cb.bucket_at)
          or (not cb.is_hourly and rev.bucket_key = cb.bucket_day)
        )
      ),
      '[]'::jsonb
    ) as daily_reviews,
        ...
        left join (
          select
            case
              ...
              else (bc.clicked_at at time zone 'utc')::date
            end as bucket_key,
            count(*)::bigint as cnt
          from public.business_clicks bc, bounds
          where bc.business_id = mb.id
          ...
          (cb.is_hourly and clk.bucket_key = cb.bucket_at)
          or (not cb.is_hourly and clk.bucket_key = cb.bucket_day)
        )
      ),
      '[]'::jsonb
    ) as daily_clicks
